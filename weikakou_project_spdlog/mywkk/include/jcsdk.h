...
typedef void (JVSDK_CALLBACKTYPE *JCDownloadDataCallback_t)(JCLink_t nLinkID, JCDownloadEventType etType, PUCHAR pData, DWORD dwSize, DWORD dwFileSize, LPVOID pUserData);
...
*			success to callback to return connectoin ID,connection result will be return in event callback,
 *			Fail to call to return JCSDK_INVALID_LINKVALUE
*/ 
...
* @return 返回调用结果 Return callback result
*/
JCSDK_API JBOOL JCSDK_CALLTYPE JCSDK_DownloadRemoteFile(JCLink_t nLinkID, int nFileID);
